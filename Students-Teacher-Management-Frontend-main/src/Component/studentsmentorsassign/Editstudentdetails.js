...
export default function StudentEditPage() {
  ...
  const formik = useFormik({
    ...
    validate :(values) => {
        ...
        }else if (!values.menterid) {
            errors.menterid = toast.error("Mention Mentor For Student");
        }
        return errors
    ...
    onSubmit : async values => {
        values = await Object.assign(values, {menterid:values.menterid});
        console.log(values);
        if(id){
          try {
            await axios.put(`https://mentor-student-assign.onrender.com/students/assignmentor/${id}`, values)
 ...
 <div style={{margin:"2%"}}>
           ...
           <div style={{marginBottom:"1%"}} className="text-start"><label>Assign Mentor</label></div>
                ...
                className="text-center"
                style={{ marginBottom: "2px", width: "100%",height:"20px", display: "flex", color: "black" }}
                {...formik.getFieldProps('menterid')}     
              >
                <option value="" disabled>---Select Mentor---</option>
                {userData.map((val)=>{
                     return( <option value={val._id}>Mentor Name: {val.name} <span style={{color:"red"}}>Specialization : {val.Specialization}</span></option>)
                })}       
              </select>
                    </div>
                    </div>
