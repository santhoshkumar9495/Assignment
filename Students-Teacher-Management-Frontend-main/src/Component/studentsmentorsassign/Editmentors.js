    ...
    const getProducts = async () => {
      ...
      setSolution(singledata.data.data[0]);
        ...
        for (let i = 0; i < tags.length; i++) {
            tagsoptions.push({
              value: tags[i]._id,
              label: `Studentid : ${tags[i]._id} , Course :${tags[i].Course} `,
            });
          }
...
// console.log(tagvalue);
  const formik = useFormik({
    ...
    onSubmit : async values => {
        values = await Object.assign(values, { studentsid:tagvalue[0].value});
        // console.log(values);
        if(id){
          try {
            await axios.put(`https://mentor-student-assign.onrender.com/mentors/assignstudent/${id}`, values)
